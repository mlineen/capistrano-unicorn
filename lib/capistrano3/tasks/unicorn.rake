      ...
      within release_path do
        with rails_env: fetch(:rails_env) do
          if test("[ -e #{fetch(:unicorn_pid)} ] && kill -0 #{pid}")
            ...
            if fetch(:unicorn_address).nil?
              execute :bundle, "exec unicorn", "-c", fetch(:unicorn_config_path), "-E", fetch(:rails_env), "-D"
            else
              execute :bundle, "exec unicorn", "-l", fetch(:unicorn_address), "-c", fetch(:unicorn_config_path), "-E", fetch(:rails_env), "-D"
            end
          end
        end
      end
